...
public class GamesPrintsViewModel : ViewModelBase
{
	...
	public bool HasAsCupCard
	{
		get => _teamBewerb.WertungskarteAsCupCard;
		set
		{
			if (_teamBewerb.WertungskarteAsCupCard == value) return;
			_teamBewerb.WertungskarteAsCupCard = value;
	...
	public ICommand PrintScoreCardsCommand => _printScoreCardsCommand ??= new RelayCommand(
		async (p) =>
		{
			var _printPreview = new PrintPreview(await ScoreCardsFactory.Create(_teamBewerb, HasNamesOnScoreCard, HasSummarizedScoreCards, HasOpponentOnScoreCard, HasAsCupCard));
