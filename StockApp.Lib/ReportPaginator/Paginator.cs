...
namespace StockApp.Lib.ReportPaginator;
...
public class Paginator : IPaginator
{
	...
	public async Task<List<UIElement>> PaginateAsync(Func<UIElement> pageFactory, Func<UIElement> tableHeaderFactory, Size pageSize, Thickness pageMargins, CancellationToken cancellationToken)
	{
		...
		_log.Debug("Start pagination loop");
		...
		while (processing)
		{
			...
			pageNumber++;
			_log.Debug($"Page {pageNumber} created");
			if (pageNumber == 1)
			{
			...
			_log.Debug($"Page { pageNumber} layout updated. Add content now");
			// Add the actual content of the page
			currentPageContainer.Content = newPage;

			_log.Debug($" do preProcessing on Page {pageNumber}");
			...
			_log.Debug($"start pagination for Page {pageNumber} with paginationTracker");
			var createNextPage = Paginate(pageLogicalChildren, paginationTracker);
			_log.Debug($"add Page {pageNumber} to processed Pages");
			processedPages.Add(currentPageContainer);

			if (!createNextPage)
			{
				_log.Debug($"do postProcessing for Page {pageNumber} ");
		...
		_log.Debug($"returning processed Pages");
	...
	private static bool PopulateItemsControl(Array items, ItemsControl itemsControl, int startIndex, out int currentItemIndex)
	{
		...
		for (var i = startIndex; i < items.Length; i++)
		{
			...
			if (itemContainer == null) return false; // added for single List
